...
Notice that we could have also defined $(z, w) = \overline{z_{1}} w_{1} + \cdots + \overline{z_{n}} w_{n}$ so that $(z, w) = z^{*} w$, but we use $(z, w) = w^{*} z$ because it is more common. 
\end{definition}
...
\begin{theorem}
A projector $P$ is independent of the basis of subspace $W$ being projected onto. 
\end{theorem}
...
\begin{lproof}
For any inner product space $V$, we have $V = E \oplus E^{\perp}$. This means that for any vector $v \in V$, $Pv = e_{1}$ where $e_{1} \in E$, and so $(I - P)v = v - e_{1} = e_{2}$ where $e_{2} \in E^{\perp}$, which means $Q = I - P$. 
...
To prove the former, suppose we have a vector $v \in \Ker A^{*} A$. We want to show that $Av = 0$. To do so, we will show its norm is 0:
$$\norm{Av}^{2} = (Av, Av) = \overline{(Av, Av)} = A^{*} (v^{*}, A^{*} v^{*}) = A^{*} (Av, v) = (A^{*} Av, v) = (0, v) = 0$$
\end{proof}
...
The above identity is sometimes used to define the adjoint operator. 
...
\begin{proof}
Suppose $B$ satisfies $(Ax, y) = (x, By) \, \forall x, y$, then we can write 
$$(Ax, y) = (x, A^{*} y) = (x, By)$$
which means $A^{*} = B$.
\end{proof}
...
We can use the idea of adjoint operators to prove 2 more properties of orthogonal projections. 
...
$$(Pv_{1}, v_{2}) = (e_{1}, e_{2} + \widetilde{e_{2}}) = (e_{1}, e_{2}) + (e_{1}, \widetilde{e_{2}}) = (e_{1}, e_{2})$$
and 
$$(v_{1}, Pv_{2}) = (e_{1} + \widetilde{e_{1}}, e_{2}) = (e_{1}, e_{2}) + (\widetilde{e_{1}}, e_{2}) = (e_{1}, e_{2})$$
Since $(Pv_{1}, v_{2}) = (v_{1}, Pv_{2})$, $P = P^{*}$. 
\end{proof}

\begin{theorem}
If $P$ is self-adjoint and $P^{2} = P$, it is an orthogonal projection. 
\end{theorem}

\begin{proof}
Note that $P$ is an orthogonal projection if and only if $(u - Pu, Pu) = 0$. We can simplify 
$$(u, Pu) = (Pu, Pu) = (u, P^{*}Pu)$$
which is true because $P$ is self-adjoint and $P^{2} = P$. 
\end{proof}
...
\begin{theorem}
An operator $U: X \rightarrow Y$ is an isometry if and only if it preserves the inner product, that is, if and only if 
$$(x, y) = (Ux, Uy) \qquad \forall x, y \in X$$
\end{theorem}

\begin{proof}
We use the polarization identities previously described. 

If $X$ is a complex space
$$
\begin{aligned}
...
If we consider the row by column perspective of matrix multiplication, the previous lemma along with the identity $U^{*}U = I$ of unitary operators means that the set of columns of $U$ are an orthonormal basis. 
...
\begin{theorem}
A matrix $A$ is unitarily equivalent to a diagonal one if and only if it has an orthogonal basis of eigenvectors. 
\end{theorem}
